...
const empty = { slug: '', name_en: '', name_es: '', description_en: '', description_es: '', icon: 'Palette', color: '#06B6D4', order: 0 }
  ...
  return (
    ...
    >
          ...
          {cats.map((c) => (
            <div key={c.id} className="rounded-3xl bg-paper border border-line p-5 flex items-center gap-4">
              ...
              <div className="flex gap-1">
                <button onClick={() => { setEditing(c); setOpen(true) }} className="w-9 h-9 rounded-full hover:bg-cream flex items-center justify-center"><Edit2 size={14} /></button>
                <button onClick={() => setConfirm(c)} className="w-9 h-9 rounded-full hover:bg-red-50 text-red-600 flex items-center justify-center"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <ConfirmModal
        ...
        message={`"${confirm?.name_en}" will be permanently removed.`}
      />
    </AdminShell>
  )
}
...
const CategoryModal = ({ open, onClose, initial, onSubmit }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial?.id ? 'Edit Category' : 'New Category'} size="md">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Input label="Slug" required><input {...register('slug', { required: true })} className="input-base" /></Input>
          <Input label="Color"><input type="color" {...register('color')} className="input-base h-12" /></Input>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Name (EN)" required><input {...register('name_en', { required: true })} className="input-base" /></Input>
          <Input label="Name (ES)" required><input {...register('name_es', { required: true })} className="input-base" /></Input>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Description (EN)"><input {...register('description_en')} className="input-base" /></Input>
          <Input label="Description (ES)"><input {...register('description_es')} className="input-base" /></Input>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Icon (Lucide name)"><input {...register('icon')} className="input-base" /></Input>
          <Input label="Order"><input type="number" {...register('order')} className="input-base" /></Input>
        </div>
