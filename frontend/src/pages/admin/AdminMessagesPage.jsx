...
export const AdminMessagesPage = () => {
  ...
  const deleteM = useMutation({ mutationFn: contactService.remove, onSuccess: () => { qc.invalidateQueries(['admin-msgs']); toast.success(t('admin.delete')) } })

  return (
    <AdminShell subtitle={t('admin.subtitle_studio')} title={t('admin.recent_messages')}>
      ...
      <div className="rounded-3xl bg-paper border border-line divide-y divide-line">
        {msgs.map((m) => (
          <div
            key={m.id}
            className={`p-6 hover:bg-subtle transition-colors ${!m.is_read ? 'bg-brand/[0.03]' : ''}`}
            onClick={() => !m.is_read && markRead.mutate(m.id)}
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="flex items-start gap-3 flex-1 min-w-[260px]">
                <div className="w-10 h-10 rounded-full bg-brand flex items-center justify-center text-paper text-sm font-medium">
                  {m.name?.[0]}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-medium">{m.name}</p>
                    {!m.is_read && <span className="w-2 h-2 rounded-full bg-brand" />}
                  </div>
        ...
        {msgs.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-steel">{t('admin.no_messages')}</p>
          </div>
        )}
      </div>
      ...
      <ConfirmModal
        ...
        title={t('admin.delete_message_confirm')}
      />
      </div>
    </AdminShell>
  )
