      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {posts.map((p) => (
          <div key={p.id} className="rounded-3xl bg-paper border border-line p-4 flex gap-4">
            <div className="w-32 h-32 rounded-2xl bg-cream overflow-hidden flex-shrink-0">
              {p.cover_image && <img src={p.cover_image} alt="" className="w-full h-full object-cover" />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2">
                <span className={`text-xs px-2 py-0.5 rounded-full ${p.is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                  {p.is_published ? 'Published' : 'Draft'}
                </span>
                {p.is_featured && <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">Featured</span>}
              ...
              <div className="flex gap-1 mt-3">
                <Link to={`/blog/${p.slug}`} target="_blank" className="w-8 h-8 rounded-full hover:bg-cream flex items-center justify-center"><Eye size={14} /></Link>
                <button onClick={() => { setEditing(p); setOpen(true) }} className="w-8 h-8 rounded-full hover:bg-cream flex items-center justify-center"><Edit2 size={14} /></button>
                <button onClick={() => setConfirm(p)} className="w-8 h-8 rounded-full hover:bg-red-50 text-red-600 flex items-center justify-center"><Trash2 size={14} /></button>
              </div>
            </div>
          </div>
        ))}
...
const PostModal = ({ open, onClose, initial, onSubmit }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial?.id ? 'Edit Post' : 'New Post'} size="lg">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Input label="Slug"><input {...register('slug', { required: true })} className="input-base" /></Input>
          <Input label="Category"><input {...register('category')} className="input-base" placeholder="general" /></Input>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Title (EN)"><input {...register('title_en', { required: true })} className="input-base" /></Input>
          <Input label="Title (ES)"><input {...register('title_es', { required: true })} className="input-base" /></Input>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Excerpt (EN)"><textarea rows={2} {...register('excerpt_en', { required: true })} className="input-base resize-none" /></Input>
          <Input label="Excerpt (ES)"><textarea rows={2} {...register('excerpt_es', { required: true })} className="input-base resize-none" /></Input>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Content (EN — supports HTML/line breaks)"><textarea rows={8} {...register('content_en', { required: true })} className="input-base resize-y" /></Input>
          <Input label="Content (ES)"><textarea rows={8} {...register('content_es', { required: true })} className="input-base resize-y" /></Input>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Input label="Read time (min)"><input type="number" {...register('read_time')} className="input-base" /></Input>
          <Input label="Tags (comma-separated)"><input {...register('tags')} className="input-base" /></Input>
