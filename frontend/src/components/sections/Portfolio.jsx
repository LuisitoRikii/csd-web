...
const MasonryItem = ({ project, idx, lang, onImageClick }) => {
  const heights = ['aspect-[3/4]', 'aspect-[4/5]', 'aspect-square', 'aspect-[5/7]', 'aspect-[3/4]', 'aspect-[4/6]']
  const aspect = heights[idx % heights.length]
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-5">
          {filtered.map((p, i) => (
            <div key={p.id} className="mb-5 break-inside-avoid">
              <MasonryItem project={p} idx={i} lang={lang} onImageClick={openLightbox} />
            </div>
          ))}
        </div>
